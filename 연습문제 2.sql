...
-- 박지성  116000
-- 추신수  86000
select c.customer_name 고객명, sum(o.amount*b.price) 총구매액
  from customer_tbl c inner join order_tbl o
    on c.customer_id = o.customer_id inner join book_tbl b
...
select c.customer_name 고객명, nvl(sum(o.amount*b.price),0) 총구매액, nvl(count(o.order_date),0) 구매횟수
  from customer_tbl c left outer join order_tbl o
...
select c.customer_name 고객명, b.book_name 책이름, o.order_date 주문일자
  from customer_tbl c inner join order_tbl o
    on c.customer_id = o.customer_id inner join book_tbl b
    on b.book_id = o.book_id
 where o.order_date = ;

select a.customer_name 고객명, a.book_name 책이름, a.order_date 주문일
  from (select row_number() over(order by o.order_date desc) rn, c.customer_name, b.book_name, o.order_date
  from customer_tbl c inner join order_tbl o
    on c.customer_id = o.customer_id inner join book_tbl b
	on b.book_id = o.book_id) a
 where rn = 1;
...
-- 고객명  책이름       책가격
-- NULL    골프 바이블  35000
select a.customer_name 고객명, a.book_name 책이름, a.price 책가격
  from (select row_number() over(order by b.price desc) rn, c.customer_name, b.book_name, b.price
    	  from customer_tbl c left outer join order_tbl o
...
-- 장미란  62000
select 고객명, pay
  from (select c.customer_name 고객명, sum(o.amount*b.price) pay, row_number() over(order by sum(o.amount*b.price) desc) rn
   	 	  from customer_tbl c inner join order_tbl o
   	 	    on c.customer_id = o.customer_id inner join book_tbl b
   	 	    on b.book_id = o.book_id
   	 	 group by c.customer_name)
