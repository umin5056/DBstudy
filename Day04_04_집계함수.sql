...
-- 평균 구하기(응시 결과가 없으면(NULL) 0으로 처리하기)
SELECT 
       AVG(NVL(KOR,0)) -- NVL을 안쓰면 NULL은 제외되어 3으로 나누게 됨
     , AVG(NVL(ENG,0))
     , AVG(NVL(MAT,0))
  FROM SAMPLE_TBL;   
  

-- 개수 구하기 : COUNT(*) 사용을 권장하고 특수한 경우만 칼럼명을 쓴다.
SELECT
       COUNT(KOR)  -- 국어 시험에 응시한 인원 수
     , COUNT(ENG)  -- 영어 시험에 응시한 인원 수 
     , COUNT(MAT)  -- 수학 시험에 응시한 인원 수
     , COUNT(*)    -- 모든 칼럼을 참조해서 한 칼럼이라도 값을 가지고 있으면 개수에 포함
  FROM SAMPLE_TBL;   
